...
const scrollHeader = () => {
	if (window.pageYOffset > 180) {
		...
		document.querySelector('.header__top').style.marginBottom = 108 + 'px';
	} else {
		document.querySelector('.header__top').style.marginBottom = 0;
...
document.querySelectorAll('.tabnav-product__item').forEach(item => {
	item.addEventListener('click', () => {
		const tabBlocks = document.querySelectorAll('.tabcontent-product__block');
		for (tabBlock of tabBlocks) {
			if (tabBlock.querySelector('.popup__photo')) {
				$(function () {
					...
					$('.slider__photo-small').slick({
						slidesToShow: 5,
						arrows: false,
						asNavFor: '.slider__photo-main',
						focusOnSelect: true,
						infinite: false,
						slidesToScroll: 1,
						responsive: [
...
const popupLinks = document.querySelectorAll('.popup-link'),
	popupPhoto = document.querySelectorAll('.popup__photo'),
...
const menuTop = document.querySelectorAll('.menu-top'),
	...
	document.querySelectorAll(".menu-item").forEach(item => {
		item.addEventListener('click', (e) => {
			if (window.innerWidth < 980 && isMobile.any() || window.innerWidth > 980 && isMobile.any()) {
				...
				if (item.querySelector('.menu-item__content').classList.contains('active')) {
					item.querySelector('.menu-item__content').classList.remove('active');
				} else {
					document.querySelectorAll(".menu-item__content.active").forEach(item => {
						item.classList.remove('active');
					});
					item.querySelector('.menu-item__content').classList.add('active');
				}
			}
		});
	});
...
function documentAction(e) {
	...
	if (!targetElement.closest('.menu-item')) {
		if (document.querySelector('.menu-item__content.active')) {
			document.querySelector('.menu-item__content.active').classList.remove('active');
		}
	}
	if (targetElement.closest('.menu-item__content')) {
		return targetElement.closest('.menu-item__content').classList.add('active');
	}
}
...
if (document.querySelector('.btn-header')) {
	...
	document.addEventListener('click', (e) => {
		...
		if (e.target.closest('.btn-header__content a')) {
			btnHeaderContent.classList.remove('active');
		}
	});
}
...
const registrationTabnavItems = document.querySelectorAll('.registration__tabnav-item');
const registrationTabcontent = document.querySelectorAll('.registration__tabcontent');
const formRegistrationSubmits = document.querySelectorAll('.registration__btn-next');

if (registrationTabnavItems.length > 0) {
	clickSubmit();
}

function clickSubmit() {
	for (formRegistrationSubmit of formRegistrationSubmits) {
		formRegistrationSubmit.addEventListener('click', (e) => {
			e.preventDefault();
			const itemClass = e.target.getAttribute('data-button');
			...
			registrationTabcontent.forEach(item => {
				item.classList.remove('active');
			});
			document.querySelectorAll(`.${itemClass}`).forEach(item => {
				item.classList.add('active');
			});
			document.querySelector(`.${complated}`).classList.add('complated');
		});
	}
}
...
// 		$('.author__card-row').mousewheel(function(e, delta) {
